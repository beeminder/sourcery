...
class CliQuals(Fixture):
    ...
    def test_end_to_end_all_three_providers_in_order(self):
        ...
        code, out, err = self.run_cli(["--repo", str(self.repo), "--output", str(out_path)])
    ...
    def test_repo_required_and_unknown_arguments_rejected(self):
        self.assertEqual(self.run_cli([])[0], 2)
        self.assertEqual(self.run_cli(["--bogus"])[0], 2)
        self.assertEqual(self.run_cli(["--repo"])[0], 2)

    def test_missing_repo_directory_rejected(self):
        code, _, err = self.run_cli(["--repo", str(self.tmp / "absent")])
    ...
    def test_existing_output_never_overwritten(self):
        self.populate()
        out_path = self.tmp / "out.html"
        out_path.write_text("precious", encoding="utf-8")
